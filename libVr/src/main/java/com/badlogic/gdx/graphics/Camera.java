...
public abstract class Camera {
	 ...
	 * viewport position and dimensions in the coordinate system expected by {@link android.opengl.GLES20#glViewport(int, int, int, int)}, with the
	 ...
	 * @param viewportHeight the screenHeight of the viewport in pixels
	...
	/** Function to translate a point given in screen coordinates to world space. It's the same as GLU gluUnProject but does not
	 * rely on OpenGL. The viewport is assumed to span the whole screen and is provided at construction time. The x- and y-coordinate
	 * of vec are assumed to be in screen coordinates (origin is the top left corner, y pointing down, x pointing to the right) as
	 * reported by the touch methods in {Input}. A z-coordinate of 0 will return a point on the near plane, a z-coordinate of
	 * 1 will return a point on the far plane.
	 * @param screenCoords the point in screen coordinates
	 * @return the mutated and unprojected screenCoords {@link Vector3} */
	public Vector3 unproject (Vector3 screenCoords) {
		unproject(screenCoords, 0, 0, screenWidth, screenHeight);
		return screenCoords;
	}
	...
	public Vector3 project (Vector3 worldCoords) {
		project(worldCoords, 0, 0, screenWidth, screenHeight);
		return worldCoords;
	}
	 ...
	 * dimensions in the coordinate system expected by {@link android.opengl.GLES20#glViewport(int, int, int, int)}, with the origin in the bottom
	...
	public Ray getPickRay (float screenX, float screenY) {
		return getPickRay(screenX, screenY, 0, 0, screenWidth, screenHeight);
	}
}
